...
run_as_hermes() {
  run sudo -u "$HERMES_USER" \
    env "XDG_RUNTIME_DIR=/run/user/$HERMES_UID" \
...
step_preflight() {
  ...
  if [[ "$DRY_RUN" == "0" ]]; then
    require_root
    require_cmds podman systemctl loginctl nft ss id useradd usermod getent
    require_cmds newuidmap newgidmap                 # from the uidmap package
...
step_enable() {
  log "Enabling + starting $SERVICE_NAME (as $HERMES_USER)"
  STARTED_SERVICE=1
  run_as_hermes systemctl --user enable --now "$SERVICE_NAME"
}
...
step_verify() {
  ...
  [dry-run] On the deployed VM, 'verify' will check:
    1. systemctl --user is-active hermes == active; Restart=always; survives podman kill
...
EOF
  ...
  local fail=0
  run_as_hermes systemctl --user is-active "$SERVICE_NAME" >/dev/null && log "  service active" || { err "  service not active"; fail=1; }
  ...
  capture_as_hermes podman exec hermes curl -m5 -s -o /dev/null https://discord.com/api/v10/gateway && log "    discord reachable" || { err "    discord NOT reachable — check DNS/egress"; fail=1; }
...
cleanup_on_failure() {
  ...
  if [[ "$STARTED_SERVICE" == "1" ]]; then
    sudo -u "$HERMES_USER" env "XDG_RUNTIME_DIR=/run/user/$HERMES_UID" \
...
main() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      all|preflight|user|podman|image|config|quadlet|nft|enable|verify|down|check) cmd="$1"; shift ;;
      --yes)     DRY_RUN=0; shift ;;
